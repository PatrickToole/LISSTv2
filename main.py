...
master.filename2 = filedialog.askopenfilename(
...
df.columns = [
    ...
    "AccelerometerY",
    "AccelerometerZ",
    "RawPressure[most significant bit]",
    "RawPressure[least significant 16 bits]",
    "AmbientLight",
    "NotUsed",
...
df_max_time = df.loc[
    (df["Hour"] == (int(h) + 1)) & (df["Minute"] <= int(m))
]  # end time
...
print(
    len(exp_window) / 40
)  # allows me to check if clipped files is accurate.aka around experiment length(60 usually)
...
if not dir_exist:
    os.mkdir(new_dir)
exp_window.to_csv(
    f"processed_data//{exp_name}//clipped {file_name}", index=False, header=True
)
...
def cont_zcons_plot(plt_color, zcons=1):
    ...
    plt.contourf(
        x, y, z, 10, cmap=plt_color
    )  # TODO - set this to a variable option with a handful of preferred colors
...
def tpc_mean_plot(num_samp_roll=4):
    ...
    if num_samp_roll == 1:
        ...
        plt.title(
            f"{exp_name} Mean Total Particle Concentration({num_samp_roll} samples) {lisst_num}"
        )
